...
#df$NP[df$year==2001 & df$country_name=="Italy" & df$party_name=="Democracy is Freedom -- The Daisy"]<-0 # merger that included new parties (https://en.wikipedia.org/wiki/Democracy_is_Freedom_%E2%80%93_The_Daisy)
...
######################################################################
#### Creating and Cleaning Collapsed Party - Cut-off at >90% vote loss 
######################################################################
df$collapse.vote.tm1<-0
df$collapse.vote.tm1[df$s.vote.change.tm1==0]<-1 # relative to the past, conservative measure

df$collapse.vote.tp1<-0
df$collapse.vote.tp1[df$s.vote.change.tp1==0]<-1 # relative to the future, counts parties as collapsing when then merge or have name changes
...
write.csv(file="EU OECD/party_data_robustness_0_9.csv", x=party.data)
