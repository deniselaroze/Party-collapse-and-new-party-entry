...
bd<-"C:/Users/André Laroze/Documents/Essex/Paper 2/Draft 1/Party politics/"
...
source("EU OECD/Party_Data_Management_May2017.R")
...
g.short$X<-NULL
party.data<-read.csv(file="EU OECD/party_data.csv")
party.data$X<-NULL
...
stargazer(sum.df, covariate.labels=c("Share Votes NP", "Successful NP (dummy)", "N.Successful NP"  ,"Log % votes collapsed party (T-2)", 
                                     ...
                                     "N. Elections", "Mean Dist. Mag.", "Turnout", "Compulsory voting (dummy)", "East Europe (dummy)", "Threshold", "ENEP"),
                                      label="tab:summary",  out=paste0(bd, "summary.tex", sep=""))
...
setEPS()
postscript("crash_location_all.eps")
...
postscript("crash_location_a0.eps")
...
summary(fe2)
fe2.pse<-coeftest(fe2, vcov=vcovHC(fe2, method="arellano"))
fe2.pse
...
vxf.pse<-coeftest(vxf, vcov=vcovHC(vxf, method="arellano"))
vxf.pse

#### Robustness test for panel corrected standared errors
...
plotreg(fe2.pse, file= paste0(bd, "main_model.png", sep=""),
        custom.model.names = "NP Success, Two-way FE",
        custom.coef.names = c("Log % votes collapsed party (t-2)", "Location collapsed party (t-2)", "Log of population", 
                              "% change in GDP", "Log inflation (+2) ", "Number of parties (t-1)",
                               "Mean Dist. Mag.", "Turnout", "Compulsory voting (Yes)",   
                             "% Vote NP (t-1)")
        )
### Printing all models in table 1

texreg(file= paste0(bd, "main_models.tex", sep=""), 
...
### Printing all models in expanded table 1 (paper) printed in appendix table 2
texreg(file= paste0(bd, "main_models_expanded.tex", sep=""), 
         list(p1.n.pse, p1.pse,p2.pse, re.pse ,fe1.pse, fe2.pse, vxf.pse) , 
       custom.model.names = c("A.M1 Pooled", "A.M2 pooled", "A.M3 pooled", "A.M4 RE", "A.M5 FE ", "A.M6  Two-way", "A.M7 Interaction " ), 
       custom.coef.names = c("Intercept", "% votes collapsed party (t-2)", "Location collapsed party (t-2)", #"Weighted Party System Dispersion",
                             "Log of population", "% change in GDP", "Log inflation (+2) ", "Number of parties (t-1)",
                             "Log time (+ 1)", "Mean Dist. Mag.", "Turnout", "Compulsory voting (Yes)",   
                             "Log % votes collapsed party (t-2)" , "Log vote share NP (t-1)" , "FPTP (dummy - Yes)", "FPTP * log % collapsed party (t-2)"),
       reorder.coef=c( 2,  12, 3, 14, 15, 4,5, 6, 7, 8, 9, 10, 11, 13, 1),
       caption = "Empirical models on the association between size of party crash, its location and interaction of size with electoral system.
      The dependent in M1 is the sum of votes obtained by new parties in the 
       first legislative election they participated. Model M2-7 use a logged version of the dependent variable (+1)",
       label="table:main_models_extended", stars = c(0.001, 0.01, 0.05, 0.1), #ci.force = T, ci.force.level = 0.95,
       booktabs = F, dcolumn = F,  sideways = F)

texreg(
  list(p1.n, p1,p2, re ,fe1, fe2, vxf), 
       custom.model.names = c("M1 Pooled", "M2 pooled", "M3 pooled", "M4 RE", "M5 FE ", "M6  Two-way", "M7 Interaction "), 
       booktabs = F, dcolumn = F,  sideways = F)

########################################################
###### Tests for Heteroskedasticity on non-logged models
########################################################


p2.nl <- plm(np.vote~ lag(share.v.crash.tp1, 2)+lag(c.location.mean.tp1,  2)#*lag(cmp.uwpsd.s, 3)
          + log(population.wdi) + gdp.pct.chg     +  log(cpi.wdi+2)  #+ unempl.wdi
          + lag(n.parties, 1)  #+ pub_fin_new
          + log(as.numeric(elect.id)+1) 
          + tier1_avemag #+ factor(fptp) 
...
plot(density(resid(p2.nl))) #A density plot
qqnorm(resid(p2.nl)) # A quantile normal plot - good for checking normality



fe2.nl <- plm(np.vote~ lag(share.v.crash.tp1, 2)+lag(c.location.mean.tp1,  2)#*lag(cmp.wpsd.s, 3)
           + log(population.wdi) + gdp.pct.chg     +  log(cpi.wdi+2)  #+ unempl.wdi
           + lag(n.parties, 1)  #+ pub_fin_new
           #+ log(as.numeric(elect.id)+1) 
           + tier1_avemag #+ factor(fptp) 
           ...
           ,data=mydf , index=c("country", "elect.id"), model="within", effect ="twoway")
summary(fe2.nl)
plot(density(resid(fe2.nl))) #A density plot
qqnorm(resid(fe2.nl)) # A quantile normal plot - good for checking normality




vxf.nl <- plm(np.vote~ factor(fptp)*lag(share.v.crash.tp1, 2)+lag(c.location.mean.tp1,  2)#*lag(cmp.wpsd.s, 3)
           + log(population.wdi) + gdp.pct.chg     +  log(cpi.wdi+2)  #+ unempl.wdi
           + lag(n.parties, 1)  #+ pub_fin_new
           #+ log(as.numeric(elect.id)+1) 
           #+ tier1_avemag #+ factor(fptp) 
           ...
           ,data=mydf , index=c("country", "elect.id"), model="within", effect ="twoway")
summary(vxf.nl)
plot(density(resid(vxf.nl))) #A density plot
qqnorm(resid(vxf.nl)) # A quantile normal plot - good for checking normality
...
### Correlation model, thin model for robustness tests
p0 <- plm(np.vote~ lag(share.v.crash.tp1, 2)+lag(c.location.mean.tp1,  2)
          #+ log(population.wdi) + gdp.pct.chg     +  log(cpi.wdi+2) # + unempl.wdi
          #+ lag(n.parties, 1)  #+ pub_fin_new
          #+ log(as.numeric(elect.id)+1) 
          #+ tier1_avemag #+ factor(fptp) 
...
p0.pse<-coeftest(p0, vcov=vcovHC(p0, method="arellano"))
p0.pse


#########################################################################
### Success of new parties other control variables, Appendix Table 3
#########################################################################
...
fe2.mc <- plm(log(np.vote+1)~ log(lag(share.v.crash.tp1, 2)+1)+lag(c.location.mean.tp1,  2)#*lag(cmp.wpsd.s, 3)
              ...
              + log(lag(np.vote, 1)+1)
              ,data=mydf , index=c("country", "elect.id"), model="within", effect ="twoway")
fe2.mc.pse<-coeftest(fe2.mc, vcov=vcovHC(fe2.mc, method="arellano"))
...
vxf.mc <- plm(log(np.vote+1)~ factor(fptp)*log(lag(share.v.crash.tp1, 2)+1)+lag(c.location.mean.tp1,  2)#*lag(cmp.wpsd.s, 3)
              ...
              + log(lag(np.vote, 1)+1)
              ,data=mydf , index=c("country", "elect.id"), model="within", effect ="twoway")
vxf.mc.pse<-coeftest(vxf.mc, vcov=vcovHC(vxf.mc, method="arellano"))
...
### Printing all models for appendix table 3
texreg(file= paste0(bd, "control_models.tex", sep=""), 
...
texreg(list(p2.mc, fe2.mc , vxf.mc ))



##########################################
### East Europe controls, Appendix table 4
##########################################
...
################## Footnote NNNNNNNNNNNNNNNNNNNNNNNN
poisson <- glm(S.NP.n~ factor(fptp)*log(share.v.crash.tp1.k2+1)+c.location.mean.tp1.k2#*cmp.wpsd.s.k3
               #+ e.eu
               + log(population.wdi) + gdp.pct.chg     +  log(cpi.wdi+2) # + unempl.wdi
               + n.parties.k1  #+ pub_fin_new
               + log(as.numeric(elect.id)+1) 
...
screenreg(list(logit, logit2, negbin, negbin2), stars = c(0.001, 0.01, 0.05, 0.1))


texreg(file= paste0(bd, "count_models.tex", sep=""), 
...
postscript("Boot_share_votes2.eps")
plot(results, index=2)
dev.off()

postscript("Boot_location2.eps")
...
postscript("boot_c_location.eps")
...
ipe <- ipe + stat_smooth(method="lm", alpha=1) + geom_point()
ipe <- ipe + scale_colour_grey(name  ="Electoral System", breaks=c(0, 1), start = 0.2, end = 0.8, labels=c("Other", "FPTP")) 
ipe <- ipe + xlab("Log % votes collapsed party (t-2) +1")+ylab("Predicted log sum NP vote share")+ theme_bw()
ipe
ggsave(filename=paste0(bd, "fptpXsize.eps", sep=""), width = 6, height = 5)


####### Plotting on a scatterplot of the observed data

lp <- ggplot(data=mydf, aes(x=log(share.v.crash.tp1.k2+1), y=log(np.vote+1), shape=factor(fptp), 
                            colour=factor(fptp))) + stat_smooth(method="lm") + geom_point()
lp <- lp + scale_colour_grey(name  ="Electoral System", breaks=c(0, 1),labels=c("Other", "FPTP"), end = 0.6)  
lp <- lp + scale_shape_discrete(name  ="Electoral System", breaks=c(0, 1), labels=c("Other", "FPTP")) 
lp <- lp + xlab("Log % votes collapsed party (t-2) +1")+ylab("Log sum NP vote share + 1")+ theme_bw()
lp

####################################################################
######## Interaction of FPTP and Location Footnote NNNNNNNNNNNNNNNNN
####################################################################

#### two-way model
iip4 <- lm(log(np.vote+1)~ log.share+c.location.mean.tp1.k2*fptp#*lag(cmp.wpsd.s, 3)
           +log(population.wdi) + gdp.pct.chg     +  log(cpi.wdi+2) # + unempl.wdi
           + n.parties.k1  #+ pub_fin_new
           #+ log(as.numeric(elect.id)+1) 
           + tier1_avemag #+ factor(fptp) 
...
###### Predicted values
ip4data <- data.frame(predict(iip4), iip4$model$c.location.mean.tp1.k2, iip4$model$fptp)

ipe4<-ggplot(ip4data, aes(x = iip4.model.c.location.mean.tp1.k2, y = predict.iip4., group = factor(iip4.model.fptp), color = factor(iip4.model.fptp))) 
ipe4 <- ipe4 + stat_smooth(method="lm" , alpha=1) + geom_point()
ipe4 <- ipe4 + scale_colour_grey(name  ="Electoral System", breaks=c(0, 1), start = 0.2, end = 0.8, labels=c("Other", "FPTP")) 
ipe4 <- ipe4 + xlab("Location (t-2)")+ylab("Predicted log sum NP vote share")+ theme_bw()
ipe4
ggsave(filename=paste0(bd, "fptpXlocation.eps", sep=""), width = 6, height = 5)
...
ipeu<-ggplot(i2data, aes(x = ilm2xe.model.c.location.mean.tp1.k2, y = predict.ilm2xe., 
                         group = factor(ilm2xe.model.e.eu), color = factor(ilm2xe.model.e.eu))) +
                          ...
                          scale_colour_grey(name  ="Region", breaks=c(0, 1), start = 0.2, end = 0.8, labels=c("Other", "East Europe")) +
                          xlab("Mean Location collapsed party (t-2)(ies)")+ylab("Predicted log sum NP vote share")+ theme_bw()
ipeu
ggsave(filename= paste0(bd, "eeuXlocation.eps", sep=""), width = 6, height = 5)
...
ipeu2<-ggplot(i2data2, aes(x = ilm2xe2.model.log.share, y = predict.ilm2xe2., 
                         group = factor(ilm2xe2.model.e.eu), color = factor(ilm2xe2.model.e.eu)))+ 
                          stat_smooth(method="lm" , alpha=1) + geom_point()+
                          scale_colour_grey(name  ="Region", breaks=c(0, 1), start = 0.2, end = 0.8, labels=c("Other", "East Europe")) +
                          xlab("Log % votes collapsed party (t-2) +1")+ylab("Predicted log sum NP vote share")+ theme_bw()
ipeu2
ggsave(filename=paste0(bd, "eeuXsize.eps", sep=""), width = 6, height = 5)
...
lm1 <- lm(log(np.vote+1)~ log.share+c.location.mean.tp1.k2
          + log(population.wdi) + gdp.pct.chg     +  log(cpi.wdi+2)  #+ unempl.wdi
          + lag(n.parties, 1)  #+ pub_fin_new
          + log(as.numeric(elect.id)+1) 
          + tier1_avemag #+ factor(fptp) 
...
lm1.pse<-coeftest(lm1, vcov=vcovHC(lm1, method="arrellano"))
lm1.pse

lm2 <- lm(diff(log(np.vote+1))~ diff(log.share)+diff(c.location.mean.tp1.k2)
          + diff(log(population.wdi)) + diff(gdp.pct.chg)     +  diff(log(cpi.wdi+2))  #+ unempl.wdi
          + diff(lag(n.parties, 1))  #+ pub_fin_new
          + diff(log(as.numeric(elect.id)+1)) 
          + diff(tier1_avemag) #+ factor(fptp) 
          + diff(turnout)
          + Compulsory.voting
          #+ thresh
          #+ lag(enep1,1)  
          + diff(log(lag(np.vote, 1)+1))
          ,data=mydf)# , index=c("country", "elect.id"), model="pooling", effect ="individual")
summary(lm2)

lm2.pse<-coeftest(lm2, vcov=vcovHC(lm2, method="arrellano"))
lm2.pse
...
hm<-heckit(selection = S.NP.d ~ log.share+c.location.mean.tp1.k2*e.eu
             + gdp.pct.chg +  log(cpi.wdi+2)
             + fptp
             + n.parties.k1
             + Compulsory.voting
             + turnout
             ...
             , outcome = log(np.vote+1)~ log.share+c.location.mean.tp1.k2*e.eu
             + gdp.pct.chg +  log(cpi.wdi+2)
             + n.parties.k1  
             + turnout
             + log(population.wdi) 
             + log(as.numeric(elect.id)+1) 
             + log(np.vote.k1+1), method = "ml", data=mydf)
summary(hm)
