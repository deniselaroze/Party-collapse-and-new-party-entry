...
############################################################
#### Creating and Cleaning Collapsed Party - Cut-off at 0.7
###########################################################
df$collapse.vote.tm1<-0
df$collapse.vote.tm1[df$s.vote.change.tm1<.3]<-1 # relative to the past, conservative measure

df$collapse.vote.tp1<-0
df$collapse.vote.tp1[df$s.vote.change.tp1<.3]<-1 # relative to the future, counts parties as collapsing when then merge or have name changes
...
write.csv(file="EU OECD/party_data_robustness_0_7.csv", x=party.data)
